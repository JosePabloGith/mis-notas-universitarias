...
int main(){
  ...
  int reloj_global = 0;
  int contador_asientos = 0; // Índice de nuestro arreglo

  // --- INICIO DE ROUND ROBIN ---
  while (cantidad_de_pacientes > 0){

    // Solo atendemos si el paciente aún necesita tiempo
    if(listaPacientes[contador_asientos].tiempo_restante > 0) {

        printf("\nLlamando al paciente %d (Le restan %d seg)\n",
               listaPacientes[contador_asientos].id_proceso,
               listaPacientes[contador_asientos].tiempo_restante);

        // [CASO 1] El tiempo que le resta es MENOR o IGUAL al Quantum
        if(listaPacientes[contador_asientos].tiempo_restante <= QUANTUM){
           printf("El paciente %d termino su tratamiento.\n", listaPacientes[contador_asientos].id_proceso);

           // RETO 4.A:
           // 1. Suma al reloj_global el tiempo que el paciente usó en este turno (¡Ojo! usó su tiempo restante, no el quantum completo).
           // 2. Iguala el tiempo_restante de este paciente a 0.
           // 3. Resta 1 a la cantidad_de_pacientes general.

        }
        // [CASO 2] El tiempo que le resta es MAYOR al Quantum
        else {
          printf("El paciente %d es atendido %d seg, pero debe volver a la fila.\n", listaPacientes[contador_asientos].id_proceso, QUANTUM);

          // RETO 4.B:
          // 1. Suma al reloj_global el QUANTUM (porque el paciente usó todo su turno completo).
          // 2. Réstale el QUANTUM al tiempo_restante del paciente.

        }
    }

    // RETO 4.C: El giro de la silla (Avanzar al siguiente paciente)
    // Escribe la lógica para sumar 1 al contador_asientos, pero si llega a 3 (PACIENTES), debe regresar a 0.
    // Pista: Puedes usar un simple 'if', o usar el operador módulo (%).

  }

  printf("\n--- SIMULACION TERMINADA ---\n");
  printf("Tiempo total tomado (Reloj Global): %d segundos\n", reloj_global);
...
void pedirPacientes(Proceso listaPacientes[]){
  for(int i = 0 ; i < PACIENTES ; i++ ){
      ...
      listaPacientes[i] = (Proceso){i+1 , numero_aleatorio, numero_aleatorio};
  }
}
